...
define le_postfix::install(
    ...
    $mysubdomain1 = $::le_postfix::params::my_subdomain_one
    
    # ensure local root goes to mail server regardless which domain host is in
    
    $mymailserverdomain = $::le_postfix::params::mymail_server_domain
    ...
    if ( $mta_type == 'server' ) {
        ...
        $server_source = $source ? {
            'UNSET' => "puppet:///modules/le_postfix/server.postfix.preseed",
            default => $source,
        }
        
        $serverpath = $::le_postfix::params::server_preseedfilepath
        ...
        file { '/etc/postfix/main.cf' :
              content =>  template( 'le_postfix/server.main.cf.erb' ),
                owner => 'root',
                group => 'root',
        ...
        if $no_lan_outbound_mail == 'true' {
            ...
            file { '/etc/postfix/transport' :
                  content =>  template( 'le_postfix/transport.erb' ),
                    owner => 'root',
                    group => 'root',
                  require => Package["postfix"],
                   notify => Service["postfix"],
        ...
        file { '/etc/postfix/master.cf' :
              content =>  template( 'le_postfix/server.master.cf.erb' ),
                owner => 'root',
                group => 'root',
              require => Package["postfix"],
               notify => Service["postfix"],
        ...
        if $server_root_mail_user != '' {
            ...
            file { '/etc/postfix/virtual' :
                  content =>  template( 'le_postfix/virtualaliases.erb' ),
                    owner => 'root',
                    group => 'root',
                  require => Package["postfix"],
            }
        ...
        'UNSET' => "puppet:///modules/le_postfix/satellite.postfix.preseed",
        default => $source,
        }
        
        $satellitepath = $::le_postfix::params::satellite_preseedfilepath
        ...
        file { '/etc/postfix/main.cf' :
              content =>  template( 'le_postfix/satellite.main.cf.erb' ),
                owner => 'root',
                group => 'root',
              require => Package["postfix"],
               notify => Service["postfix"],
        ...
        file { '/etc/aliases' :
              content =>  template( 'le_postfix/aliases.erb' ),
                owner => 'root',
                group => 'root',
              require => Package["postfix"],
               notify => Service["postfix"],
