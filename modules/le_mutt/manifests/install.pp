...
##
##  Sample use:
##  	le_mutt::install { 'root': mailserver_hostname => 'rohan' }
##      le_mutt::install { 'bekr': mailserver_hostname => 'rohan' }
##
define le_mutt::install ( $mailserver_hostname='' ) {
    ...
    if ( $::hostname == $mailserver_hostname ) or ( $name == 'root' ) {
        $mailspool = $::le_mutt::params::localmailspool
        $mailfolder = $::le_mutt::params::localmailfolder
        
        $imap_user = $::le_mutt::params::no_imap_user
        $imap_passwd = $::le_mutt::params::no_imap_passwd
        ...
        $imap_passwd = "set imap_pass = $::le_mutt::params::mytestpasswd"
    ...
    if $name == 'root' {
        ...
        file { "/root/.mutt/muttrc" : 
            content =>  template( 'le_mutt/muttrc.erb' ),
              owner => 'root',
              group => 'root',
