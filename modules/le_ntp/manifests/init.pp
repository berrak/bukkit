...
## Sample usage:
##
##   class { 'le_ntp' : role => 'lanclient', peerntpip => $ipaddress }
##   class { 'le_ntp' : role => 'lanserver', peerntpip => '192.168.0.1' }
##
class le_ntp(
    $role='lanclient',
    $peerntpip='UNSET',
    ...
    case $role {
        ...
        lanclient:  {
            ...
            file { "ntp.conf.lanclient":
                path => "/etc/ntp.conf",
                source => "puppet:///modules/le_ntp/ntp.conf.lanclient",
                owner => "root",
                group => "root",
                require => Package["ntp"],
                notify => Service["ntp"],
        ...
        lanserver:  {
            ...
            file { "ntp.conf.lanserver":
                path => "/etc/ntp.conf",
                source => "puppet:///modules/le_ntp/ntp.conf.lanserver",
                owner => "root",
                group => "root",
                require => Package["ntp"],
                notify => Service["ntp"],
    ...
    if $peerntpip == 'UNSET' {
        ...
        file { '/root/bin/ntp.time' :
            content =>  template( 'le_ntp/ntp.time.erb' ),
               mode => '0700',
              owner => 'root',
              group => 'root',
            require => Package["ntp"], 
